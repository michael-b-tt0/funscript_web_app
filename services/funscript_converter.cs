...
{
    public static Funscript GetQuarterSpeedScript(Funscript funscript, int quarterSpeedFactor)
    {
        if (funscript == null)
        {
            return new Funscript
            {
                actions = Array.Empty<ActionData>(),
                title = "NullInput_QuarterSpeed"
            };
        }

        if (funscript.actions == null)
        {
            return new Funscript
            {
                version = funscript.version,
                ...
                metadata = funscript.metadata,
                video_url = funscript.video_url,
                title = $"{funscript.title}_QuarterSpeed_NullActions"
            };
        }

        ActionData[] modifiedActions = funscript.actions.Select(action => new ActionData
        {
            at = action.at * quarterSpeedFactor,
            pos = action.pos
        }).ToArray();
        ...
        Funscript quarterSpeedScript = new Funscript
        {
            ...
            metadata = funscript.metadata,
            video_url = funscript.video_url,
            title = $"{funscript.title}_QuarterSpeed"
        };
...
public static class funscript_converter_fundoubler
{
    ...
    public static Funscript GetDoubleSpeedScript(Funscript funscript, FunDoublerOptions options)
    {
        ...
        var actionGroups = longFirstWait
            ? funscript.GetActionGroups(orderedActions.Skip(1).ToArray())
            : funscript.GetActionGroups(orderedActions.ToArray());
