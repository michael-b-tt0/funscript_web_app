...
public class FunSmootherOptions
{
    ...
    public int ReductionStrength { get; set; } = 100;
}

public static class funscript_converter_funsmoother
{
    ...
    public static List<ActionData> RemoveShortPauses(List<ActionData> filteredGroup, FunSmootherOptions options)
    {
    ...
    public static List<KeyAction> IdentifyKeyActions(List<ActionData> filteredGroup, FunSmootherOptions options)
    {
    ...
    public static ActionData[] GenerateFinalActions(List<KeyAction> keyActions, Funscript funscipt, FunSmootherOptions options)
    {
        ...
        for (int i = 0; i < keyActions.Count; i++)
        {
            ...
            else
            {
                ...
                {
                    double r = options.ReductionStrength / 100.0;
                    int originalPos = action.pos;
                    double reducedPos = (double)(lastAction.SubActions.Sum(a => a.pos) + action.pos) / (lastAction.SubActions.Count + 1);
                    double newPos = originalPos * (1 - r) + reducedPos * r;
    ...
    public static Funscript GetSmoothedSpeedScript(Funscript funscript, FunSmootherOptions options, ILogger logger)
    {
        ...
        Funscript SmoothedSpeedScript = new Funscript
        {
            ...
            title = $"{funscript.title}_Smoothed_with_pause_duration{options.ShortPauseDuration}"
        };

        return SmoothedSpeedScript;
    }
